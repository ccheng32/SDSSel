...
vector<vector<double>> ediv,esoc,epef;
 ...
 */
 void init()
 {
  ...
  esoc = vector<vector<double>> (v+1);
  epef = vector<vector<double>> (v+1);
 	REP(i,0,c) ediv[i] = vector<double>(c+1);
  REP(i,0,v) esoc[i] = vector<double>(v+1);
  REP(i,0,v) epef[i] = vector<double>(c+1);
 ...
 * @return pair<vector<int>,vector<int>>: 第一個vector是h-hop裡面的viewer, 第二個是對應到的channel(有刪掉不符合constraint的)
 */
vector<int>> get_h2(int cur)
{
	queue<int> q;
  vector<int> r1;
	vector<int> dis2(n+1);
	...
	for(auto i:e[cur]) if(i.F<=v && !dis2[i.F])
	{
		dis2[i.F] = dis2[cur]+1;
		q.push(i.F);
	}
...
vector<int> get_viewers(vector<int> &ch)//
{
	...
	for(int i:ch) for(auto j:e[i]) if(j.F<=v) viss[j.F]=true;
...
vector<int> get_channels(vector<int> &vi)//
{
	vector<int> re;
	vector<bool> viss(n+1);
	for(int i:vi) for(auto j:e[i]) if(j.F>v) viss[j.F]=true;
...
//	for(int i:x) visx[i]=true;
	for(int i:x) if(i>v) for(int j:x) if(j>v) re += ediv[i-v][j-v];
//	for(int i:x) for(auto j:ediv[i]) if(!del[j.F] && visx[j.F] && j.F>v) re += j.S;
...
//	for(int i:x) visx[i]=true;
	for(int i:x) if(i>v) for(int j:x) if(j>v) re += ediv[i-v][j-v];
//	for(int i:x) for(auto j:ediv[i]) if(!del[j.F] && visx[j.F] && j.F>v) sum[i] += j.S;
//	REP(i,v+1,v+c)
//	{
//		re += sum[i];
//	}
...
void shared_apx(vector<int> kapxthread, vector<int> fapxthread, double kapxthreadobj,int vbthread, int Proc, int num){
...
void sdsselthread(int startrange, int endrange, int threadnum)
{
	...
	int vb=-1, qb=-1;
	vector<int> h2v;
	vector<int> kapx, fapx, lall;
  bool all = false;
  int Proc=0;
	//vector<int> kapximp, fapximp;
	//vector<int> cfit;
  REP(i,1,c) lall.PB(i);
	REP(Vs,startrange,endrange)
	{
		//new cut added
    vector<bool> ldel_op, ldel_all, div_op, div_all;
    double lavg_op = 0, lavg_all = 0;
    int min_op, min_all;
		...
		//{
			//continue;
		//}
    ldel_op = vector<bool> (v+1);
    ldel_all = vector<bool> (v+1);
    REP(i,1,v+1) ldel_op[i] = true;
    REP(i,1,v+1) ldel_all[i] = true;
		h2v = get_h2(Vs);
    ...
    min_op = 1;
    int lnb = 0;
    for (int i:h2v) if(avp_op[i]<avp_op[min_op] && !ldel_op[i]) min_op = i;
    for (int i:h2v) if(!ldel_op[i] && i != min_op)
    {
      lavg_op = lavg_op + avp_op[i];
      lnb=lnb+1;
    }
    lavg_op = lavg_op / lnb;
    while(lavg_op>avp_op[min_op] && lnb>P)
    {
      ldel_op[min_op] = true;
      lnb = 1;
      for (int i:h2v) if(avp_op[i]<avp_op[min_op] && !ldel_op[i]) min_op = i;
    ...
    lavg_op = ((lavg_op*lnb)+avp_op[min_op])/lnb;
    min_all = 1;
    int lnb_all = 1;
    for (int i:h2v) if(avp_all[i]<avp_all[min_all] && !ldel_all[i]) min_all = i;
    for (int i:h2v) if(!ldel_all[i] && i != min_all)
    {
      lavg_all = lavg_all + avp_all[i];
      lnb_all=lnb_all+1;
    }
    lavg_all = lavg_all / lnb_all;
    while(lavg_all>avp_all[min_all] && lnb_all>P)
    {
      ldel_all[min_all] = true;
      lnb_all = 0;
      for (int i:h2v) if(avp_all[i]<avp_all[min_all] && !ldel_all[i]) min_all = i;
      for (int i:h2v) if(!ldel_all[i] && i != min_all)
      {
    ...
    lavg_all = ((lavg_all*lnb_all)+avp_all[min_all])/lnb_all;
    div_op = total_diversity(OP)/OP.size();
    div_all = total_diversity(lall)/lall.size();
    if (kapx.size()==0)
    {
      if (lavg_op+div_op<lavg_all+div_all)
      {
        kapx = lall;
        vector <int> trans;
        for (int i:h2v) if(!ldel_all[i]) trans.PB(i);
        fapx = trans;
        kapxobj = lavg_all+div_all;
        vb = Vs;
      }
      else
      {
        kapx = OP;
        vector <int> trans;
        for (int i:h2v) if(!ldel_op[i]) trans.PB(i);
        fapx = trans;
        kapxobj = lavg_op+div_op;
        vb = Vs;
      }
    }
    else
    {
      if (lavg_op+div_op<lavg_all+div_all && lavg_all+div_all>kapxobj)
      {
        kapx = lall;
        vector <int> trans;
        for (int i:h2v) if(!ldel_all[i]) trans.PB(i);
      ...
      if (lavg_op+div_op>=lavg_all+div_all && lavg_op+div_op>kapxobj)
      {
        kapx = OP;
        vector <int> trans;
        for (int i:h2v) if(!ldel_op[i]) trans.PB(i);
...
void IMP(){
  ...
  if(!SKIP_IMP)
	{
		...
		{
			double s=0;
			for(auto j:e[i]) if(j.F>v && lower_bound(h2c.begin(), h2c.end(), j.F)!=h2c.end())
				s += j.S;
			if(s < kapxobj)
				tp.PB(i);
		}
  ...
  //printf("%f\n", timespec2msec(&edthread) - timespec2msec(&stthread));
	printf(" CPUTime main thread %f, CPUTime %f, Wall clock time %f, Viewer V cap %d, Preproc %d, %d %d %f %d %f\n",
		timespec2msec(&edthread) - timespec2msec(&stthread),
  ...
  printf("%d %d\n", (int)fapx.size(), (int)kapx.size());
	for(int i:Proc_threads) printf("%d ",i); puts("");
...
{ //Now you have to multithread this part
  st = clock();
  ...
  avp_op = vector<double> (v+1);
  avp_all = vector<double> (v+1);
  REP(i,0,V-3)
  {
    ndel[i] = false;
  ...
  while(iter)
  {
    ...
    REP(i,0,V-3)
    {
      REP(j,0,V-3)
      {
        if(!ndel[i] && !ndel[j])
        {
          graph[i][j] = ediv[i+1][j+1];
          graph[j][i] = ediv[i+1][j+1];
      ...
      cout << "induced weight = " << indiv[i] << endl;
    }
    int k=0;
  ...
  REP(i,0,V-3) if (!ndel[i]) OP.PB(i+1);
  REP(i,1,v)
  {
    avp_op[i] = 0;
    avp_all[i] = 0;
    for (int j:OP) avp_op[i] = avp_op[i]+epef[i][j];
    avp_op[i] = avp_op[i]/OP.size();
    REP(j,1,c)  avp_all[i] = avp_all[i]+epef[i][j];
    avp_all[i] = avp_all[i]/c;
  }
  //preprocessing();
  nbrV =0;
